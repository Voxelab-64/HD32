...
static en_result_t SetUartBaudrate(uint32_t u32Baudrate)
{
    ...
    u32UartClk = u32PClk1 / (1ul << (2ul * (M4_USART2->PR_f.PSC)));
    ...
    else
    {
        ...
        else
        {
            ...
            M4_USART2->CR1_f.FBME = (0xFFFFFFFFul == DIV_Fraction) ? 0ul : 1ul;
            M4_USART2->BRR_f.DIV_FRACTION = DIV_Fraction;
            M4_USART2->BRR_f.DIV_INTEGER = DIV_Integer;
            enRet = Ok;
        }
    }
    return enRet;
...
en_result_t Ddl_UartInit(void)
{
    ...
    M4_USART2->CR1_f.ML = 0ul;    // LSB
    M4_USART2->CR1_f.MS = 0ul;    // UART mode
    M4_USART2->CR1_f.OVER8 = 1ul; // 8bit sampling mode
    M4_USART2->CR1_f.M = 0ul;     // 8 bit data length
    M4_USART2->CR1_f.PCE = 0ul;   // no parity bit
    ...
    else
    {
        ...
        M4_USART2->CR1_f.TE = 1ul;    // TX enable
    }
