...
inline void Draw_Menu_Line(const uint8_t line, const uint8_t icon=0, const char * const label=nullptr) {
	delay(1);
  if (label) DWIN_Draw_String(false, true, font8x16, Black, Background_black, LBLX, 52 + line * MLINE, (char*)label);
	DWIN_UpdateLCD();
  ...
  if (encoder_diffState != ENCODER_DIFF_NO) {
    if (encoder_diffState == ENCODER_DIFF_CW) {
      ...
      // if (!planner.is_full()) {
        ...
        sprintf_P(cmd, PSTR("G1 E%lf F50"),  HMI_ValueStruct.Move_E_scale/10+last_E_scale);
  ...
  if (encoder_diffState != ENCODER_DIFF_NO) {
    if (encoder_diffState == ENCODER_DIFF_CW) {
      ...
      else if (HMI_flag.step_flag == E_AXIS) 
     {
      ...
      DWIN_Draw_FloatValue(true, true, 0, font8x16, Black, Background_black, 4, 1, 400, MBASE(4 + MROWS -index_step), HMI_ValueStruct.Max_Step);
      }
      return;
    }
    // Step limit
    ...
    {
//      NOMORE(HMI_ValueStruct.Max_Step, 1000);
			if(HMI_ValueStruct.Max_Step > 9999)
			{
				 HMI_ValueStruct.Max_Step = 9999;
			}
      NOLESS(HMI_ValueStruct.Max_Step, MIN_STEP);
    // Step value
    DWIN_Draw_FloatValue(true, true, 0, font8x16, White, Select_Color, 4, 1, 400, MBASE(4 + MROWS -index_step), HMI_ValueStruct.Max_Step);
    }
  }
}                                                                                 
...
inline void Draw_SDItem(const uint16_t item, int16_t row=-1) {
  ...
  make_name_without_ext(str, name,20);
  if(card.flag.filenameIsDir)
  {
    return;
  }
...
inline void Draw_Move_Menu() {
 ...
 // 
  if (HMI_flag.language_flag) {
    DWIN_Frame_AreaCopy(1,156,50,200,66,285,12);//轴移�?    //DWIN_Frame_AreaCopy(1, 192, 1, 271 - 38, 479 - 465, 14, 8);
...
void Draw_Bltouch_Menu(void)
{
      ...
      if(HMI_flag.language_flag)
      {
          ...
          DWIN_ICON_Show(ICON1,27,240,80);                 //16�?
          DWIN_Frame_AreaCopy(1, 124, 50, 150,64, 170,225);  //返回       
      }
      else
      {
          ...
          DWIN_ICON_Show(ICON,25,160,225);                 //返回箭头
          DWIN_ICON_Show(ICON1,27,240,80);                 //16�?          DWIN_ICON_Show(ICON1,31,140,50);                 
          DWIN_Frame_AreaCopy(1, 124, 50, 150,64, 170,225);  //返回 
      }
...
inline void Draw_Steps_Menu() {
  ...
  DWIN_Draw_FloatValue(true, true, 0, font8x16, Black, Background_black, 4, 1, 400, MBASE(4), planner.settings.axis_steps_per_mm[E_AXIS] * MINUNITMULT);
}
...
void DWIN_CompletedMaticHate(void)
{
      if(checkkey==Feeding) 
      {
        ...
        gcode.process_subcommands_now_P(PSTR("G92 Z0 E0"));
        gcode.process_subcommands_now_P(PSTR("G1 F800 Z10"));
        gcode.process_subcommands_now_P(PSTR("G1 F500 E350"));
        gcode.process_subcommands_now_P(PSTR("G1 F100 E500"));
      ...
      if(checkkey==Desizing) 
      {
        ...
        gcode.process_subcommands_now_P(PSTR("G92 Z0 E0"));
        gcode.process_subcommands_now_P(PSTR("G1 F800 Z10"));
        gcode.process_subcommands_now_P(PSTR("G1 F500 E50"));
        gcode.process_subcommands_now_P(PSTR("G1 F800 E-600")); 
        gcode.process_subcommands_now_P(PSTR("M400"));
        gcode.process_subcommands_now_P(PSTR("M104 S0"));    
      }
       DWIN_UpdateLCD();
